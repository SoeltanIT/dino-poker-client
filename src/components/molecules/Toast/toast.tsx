...
export const Toast = (props: ToastProps) => {
  let { type = 'info', icon = '', message = '---', id, duration = 3000, lang } = props
  ...
  return (
    <div
      ...
      <div className="text-xs font-light flex-grow p-3">{message}</div>
      <button
        aria-label="Close"
        onClick={() => {
          remove(id, wrapperRef)
        ...
        <span className="sr-only">{lang?.common?.close}</span>
        {closeIcon}
      </button>
    </div>
  )

...
export interface AnnouncementListingProps {
  lang?: LangProps
  locale?: Locale
  initialData?: AnnouncementDTO[]
}

export function AnnouncementListing({ lang, locale, initialData }: AnnouncementListingProps) {
    ...
    <div className='min-h-screen flex flex-col w-full text-app-text-color px-6 lg:px-16 my-10'>
      <div className='container mx-auto flex flex-col'>
        ...
        <div className='mb-[36px]'>
          <h1 className='text-3xl font-bold text-app-text-color uppercase'>{lang?.common?.announcements}</h1>
        </div>
        {/* Desktop Layout */}
          ...
          <div className='flex-1 flex flex-col gap-4'>
            {initialData && initialData?.length > 0 ? (
              ...
              <div className='flex flex-col items-center justify-center w-full h-[50vh]'>
                <ImageIcon width={75} height={75} className='text-app-text-color' />
                <span className='mt-3'>{lang?.common?.noAnnouncementFound}</span>
              </div>
            )}
          </div>
        </div>
                  ...
                  <Card className='cursor-pointer transition-opacity bg-app-white100 overflow-hidden'>
                    <div className='aspect-video w-full'>
                      {item?.image ? (
                        <Image
                          src={item.image}
                          alt={`Announcement-${item.image}-Mobile`}
                          width={150}
                          height={150}
                          className='w-full h-full object-cover'
                        />

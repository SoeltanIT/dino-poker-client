import { baiJamjuree, notoSansKR } from '@/app/font'
import { Providers } from '@/components/providers/Providers'
import AdjustmentAppWrapper from '@/components/templates/App/AdjustmentAppWrapper'
    ...
    <html lang={params.lang} suppressHydrationWarning>
        ...
        function uVph() {
          if (window.innerWidth < 768) {
            document.documentElement.style.setProperty("--vp-height", window.innerHeight + "px");
              ...
              document.documentElement.style.setProperty("--vp-height", "100dvh");
          }
        }
      ...
      <body className={`${baiJamjuree.className} ${notoSansKR.variable}`}>
        <Providers initialFeatures={initialFeatures}>
          ...
          <LiveChatProvider licenseId={configMap['livechat_id']}>
            <AdjustmentAppWrapper locale={params.lang} lang={lang} config={configMap} features={initialFeatures}>
              <div className='min-h-[calc(var(--vp-height)-100px)] flex flex-col w-full text-app-text-color px-6 lg:px-20 py-10 mx-auto'>
                {children}
              </div>
            </AdjustmentAppWrapper>
          </LiveChatProvider>
          {/* </AppTemplate> */}
          <ToastContainer />
        </Providers>

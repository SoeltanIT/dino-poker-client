import { DetailAffiliateHistoryOthersByUser } from '@/components/organisms/Affiliate/AffiliateDetail/AffiliateHistoryOthersByUser/DetailAffiliateHistoryOthersByUser'
import { getDictionary, getLocale } from '@/dictionaries/dictionaries'
import { Locale } from '@/i18n-config'
import { ArrowLeft } from 'lucide-react'
import Link from 'next/link'

export default async function DetailAffiliateHistoryOthersByUserPage({
  params
}: {
      ...
      <div className='flex flex-col'>
        ...
        <div className='flex lg:flex-row flex-col mb-4 lg:mb-0 items-center justify-between'>
          <div className='w-full lg:mb-8 mb-2'>
            <Link
              href={`/${locale}/affiliates/others/${userId}`}
            ...
            <div className='flex items-center gap-2 justify-between w-full flex-wrap'>
              <h1 className='text-2xl font-bold uppercase'>{dict?.affiliate?.detailAffiliateHistoryCasinoByUser}</h1>
            </div>
          </div>
        </div>
        <DetailAffiliateHistoryOthersByUser userId={userId} lang={dict} />
      </div>
    </div>
  )
}

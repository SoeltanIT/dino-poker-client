import { AffiliateHistoryOthersByUser } from '@/components/organisms/Affiliate/AffiliateDetail/AffiliateHistoryOthersByUser'
import { getDictionary, getLocale } from '@/dictionaries/dictionaries'
import { Locale } from '@/i18n-config'
import { ArrowLeft } from 'lucide-react'
import Link from 'next/link'

export default async function AffiliateHistoryOthersByUserPage({
  params
}: {
        ...
        <div className='flex lg:flex-row flex-col mb-4 lg:mb-0 items-center justify-between'>
          <div className='w-full lg:mb-8 mb-2'>
            ...
            <div className='flex items-center gap-2 justify-between w-full flex-wrap'>
              <h1 className='text-2xl font-bold uppercase'>{dict?.affiliate?.affiliateHistoryCasinoByUser}</h1>
            </div>
          </div>
        </div>
        <AffiliateHistoryOthersByUser userId={userId} lang={dict} />
      </div>
    </div>
  )
}
